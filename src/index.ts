...
import { AuthHandler, Passauth } from "passauth";
...
export const PassauthExpress = (
  ...
  router.post("/login", async (req, res) => {
    try {
      ...
      const result = await handler.login(data, "role");
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        const { id } = RevokeRefreshTokenValidator.parse(req.body);

        const handler = passauth.handler;

        handler.revokeRefreshToken(id);
  ...
  if (emailConfig) {
    router.get("/reset-password", async (req, res) => {
      try {
        ...
        const { success, error } = await handler.sendResetPasswordEmail(email);
